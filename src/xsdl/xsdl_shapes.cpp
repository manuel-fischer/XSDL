...
namespace xsdl
{
    ...
    pen pen::create_pen(class renderer& renderer, color color, int width, bool premultiply)
    {
    ...
    pen pen::create_smooth_pen(class renderer& renderer, color color, int width, bool premultiply)
    {
        // 1+x^4 - 2x^2
        ...
        for(size_t i = 1; i < width+1; ++i)
        {
            ...
            line_pixels[i] = (class color){color.r, color.g, color.b, int8_t(alpha)}.premultiply_if(premultiply);
        }
        line_pixels.front() = line_pixels.back() = (class color){color.r, color.g, color.b, 0}.premultiply_if(premultiply);
    ...
    void pen::draw_line(float x0, float y0, float x1, float y1)
    {
        float length = std::hypotf(y1-y0, x1-x0);
        float angle  = atan2f(y1-y0, x1-x0);
